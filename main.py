...
df_x = pd.DataFrame(
    connection.query("select age, capitalgain, capitalloss, hoursperweek from censusdata where id < 100"))
df_y = pd.DataFrame(connection.query(
    "select age, hoursperweek from censusdata where id = 3 or  id = 4 or  id = 7  or  id = 22  or  id = 55"))
